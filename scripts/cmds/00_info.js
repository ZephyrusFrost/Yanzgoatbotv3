const fs = require('fs'); const moment = require('moment-timezone'); module.exports = { config: { name: "info", version: "1.0", countDown: 20, role: 0, shortDescription: { vi: "", en: "" }, longDescription: { vi: "", en: "" }, category: "owner", guide: { en: "owner" }, envConfig: {} }, onStart: async function ({ message }) { const botName = "yanzu"; const botPrefix = "×"; const authorName = "Kyle"; const ownAge = "19"; const teamName = "Github team"; const authorFB = "https://www.facebook.com/Kylethepogingnilalang"; const authorInsta = "kyleigopsecrets"; const tikTok = "https://www.tiktok.com/@itzkylesecrets?_t=8iwD31fPwHJ&_r=1"; const urls = JSON.parse(fs.readFileSync('cliff.json')); const link = urls[Math.floor(Math.random() * urls.length)]; const now = moment().tz('Asia/Jakarta'); const date = now.format('MMMM Do YYYY'); const time = now.format('h:mm:ss A'); const uptime = process.uptime(); const seconds = Math.floor(uptime % 60); const minutes = Math.floor((uptime / 60) % 60); const hours = Math.floor((uptime / (60 * 60)) % 24); const days = Math.floor(uptime / (60 * 60 * 24)); const uptimeString = `${days} days ${hours} hours ${minutes} minutes ${seconds} seconds`; message.reply({ body: `《  Bot & Owner Info 》
\Name: ${botName}
